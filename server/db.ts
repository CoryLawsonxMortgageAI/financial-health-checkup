...
import { InsertUser, users, submissions, InsertSubmission } from "../drizzle/schema";
...
/**
 * Create a new financial health submission
 */
 ...
 */
export async function updateSubmissionEmailStatus(id: number, emailSent: boolean) {
  const db = await getDb();
  if (!db) {
    throw new Error("Database not available");
  }
